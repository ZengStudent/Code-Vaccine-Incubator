import os, shutil
import random
from AST_Program.Exponential_Search.SourceCode import Exponential_Search as amutant
...
def create_testdata():
    ...
    for i in range(1, numbers + 1):
        ...
        # output_data
        temp_output = amutant.exponentialSearch(input_data[str(i)],len(input_data[str(i)])-1,aim[str(i)])
...
def create_testcase():
    ...
    def setUp(self):'''
        ...
        self.input_maxlen = ''' + str(len(value)-1) + '''
    ...
    def test_tri(self):
        ...
        # part four
        part_four = '''exponentialSearch(self.input_data,self.input_maxlen,self.aim),''' + str(output_data[name]) + ')'
        # part five
        part_five = '''\n            print('Pass')
            self.result='Pass'
