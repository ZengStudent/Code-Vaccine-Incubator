import os, shutil
import random
from AST_Program.Fibonacci_Number.SourceCode import Fibonacci_Number as amutant
...
def create_testdata():
    ...
    for i in range(1, numbers + 1):
        ...
        for j in range(1, input_numbers + 1):
            temp_input = i
            input_data[str(i)] = temp_input
        print(input_data[str(i)] )
        # output_data
        temp_output = amutant.fib(input_data[str(i)])
...
def create_testcase():
    ...
    def test_tri(self):
        ...
        # part four
        part_four = '''fib(self.input_a),''' + str(output_data[name]) + ')'
        # part five
        part_five = '''\n            print('Pass')
            self.result='Pass'
