...
# 輸出測試案例殺死HOM的路徑
test_killed_hom_dir = 'AST_TestCase_Killed_Mutant_Folder/AST_HOM_Killed_TestCase_Folder'
...
def get_trail_dir(root_dir,trail_dir):
    for name in os.listdir(root_dir):
        if (os.path.isdir(root_dir + name)):
            ...
            name = name.replace('_HOM_TestCase_Result_Folder_','_TestCase_')
...
def convert_HOM_killed_testcase():
    ...
    # 同時讓TestCase紀錄可殺死的 HOM
    for name,killed in temp_HOM_killed_testcase_after.items():
        for testnumber in killed:
            testcase_killed_dict['AST_TestCase_' + str(testnumber)].append(name)
...
# 寫入各個測試案例可以殺死的 HOM
def write_killed_hom(dir,testcase_killed_dict):
...
# 寫入各個測試案例可以殺死的 HOM
write_killed_hom(test_killed_hom_dir,testcase_killed_dict)
